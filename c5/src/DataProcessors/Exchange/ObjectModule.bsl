...
var SenderCode export;
var SenderName export;
var Sender;
var ЗаписьСообщения;
var УзелОбменаЗагрузкаДанных;
var мНомерВходящегоСообщения;
...
Функция одТипЗначенияСтрокой(ЗначениеИлиТип) Экспорт
	...
	Иначе
		...
		Если Менеджер = Неопределено Тогда
			...
			Текст= НСтр("en='Неизвестный тип:';ru='Неизвестный тип:'") + Строка(ТипЗнч(ТипЗначения));
...
Процедура ИнициализироватьВедениеПротоколаОбмена() Экспорт
	...
	мФайлПротоколаДанных = Новый ЗаписьТекста(ИмяФайлаПротоколаОбмена, , , ДописыватьДанныеВПротоколОбмена) ;
...
Процедура ВыгрузитьОбработчикиСобытий(Отказ) Экспорт
	...
	Если ЭтоИнтерактивныйРежим Тогда
		...
		Если ФлагОшибки Тогда
			...
			ТекстПредупреждения = НСтр("en='При выгрузке обработчиков были обнаружены ошибки.';ru='При выгрузке обработчиков были обнаружены ошибки.'");
			СообщитьПользователю(ТекстПредупреждения);
			
		Иначе
			
			ТекстПредупреждения = НСтр("en='Обработчики успешно выгружены.';ru='Обработчики успешно выгружены.'");
			СообщитьПользователю(ТекстПредупреждения);
...
Процедура УстановитьОбменДаннымиЗагрузка(Объект, Значение = Истина) Экспорт
	...
	if ( SenderCode <> undefined ) then
		if ( Sender = undefined ) then
			Sender = ExchangePlans [ SenderName ].FindByCode ( SenderCode );
		endif; 
		try
			Объект.ОбменДанными.Отправитель = Sender;
		except
		endtry;
	endif;
	
	if ( Exchange ) then
		Если УзелОбменаЗагрузкаДанных <> Неопределено И НЕ УзелОбменаЗагрузкаДанных.Пустая() Тогда
			Попытка
...
Процедура УстановитьСвойстваЗаписи(Объект, Запись, ИнформацияОТипах, 
	...
	Пока ФайлОбмена.Прочитать() Цикл
		...
		Если ИмяУзла = "Свойство"
			...
			fullName = Метаданные.НайтиПоТипу(ТипЗнч(Запись)).ПолноеИмя();
			standard = Найти ( fullName, "AccumulationRegister" ) or Найти ( fullName, "РегистрНакопления" );
			Если Имя = "ВидДвижения" И standard Тогда
...
Процедура ЗагрузитьДвижения(Объект, Имя, Очистить, ОбщаяИнформацияОТипеДокумента, НужноЗаписатьОбъект, 
	...
	КонецЦикла;
	Движения.Write = НужноЗаписатьОбъект;
...
Функция ВыполнитьВыгрузкуИзмененныхДанныхДляУзлаОбмена(УзелОбмена, МассивПравилКонвертации, СтруктураДляУдаленияРегистрацииИзменений)
	...
	Попытка
		...
		if ( Exchange ) then
			ЗаписатьИнформациюОбОбменеДаннымиЧерезПланыОбмена ( УзелОбмена );	
		endif;
		...
		Пока ВыборкаИзменений.Следующий() Цикл
			...
			Если ТекущаяСтрокаПравилаВыгрузки <> Неопределено Тогда
				...
				Если ВыгружаетсяРегистр Тогда
					...
					if ( Metadata.InformationRegisters.Contains ( ТекущийОбъектМетаданных )
						and ТекущийОбъектМетаданных.WriteMode = Metadata.ObjectProperties.RegisterWriteMode.Independent ) then
						rule = ТекущийОбъектМетаданных.Name + "Deletion";
						if ( Правила.Property ( rule ) ) then
							rk = GetRecordKey ( ОбъектДляПравилВыгрузки );
							ВыгрузитьПоПравилу ( rk, , , , rule );
						endif; 
					endif; 
...
Процедура ОткрытьФайлЗагрузки(ТолькоПрочитатьШапку=Ложь, ДанныеФайлаОбмена = "") Экспорт
	...
	Если ПустаяСтрока(ИмяФайлаОбмена) И ТолькоПрочитатьШапку Тогда
		ДатаНачала         = undefined;
		ДатаОкончания      = undefined;
		ДатаВыгрузкиДанных = undefined;
		ВерсияПравилОбмена = undefined;
...
Процедура ЗагрузитьЗначенияПараметровОбменаДанными()
	...
	Если СобытияПослеЗагрузкиПараметров.Свойство(Имя, АлгоритмПослеЗагрузкиПараметра)
		...
		Если ФлагРежимОтладкиОбработчиков Тогда
			...
			ВызватьИсключение НСтр("en='Отладка обработчика ""После загрузки параметра"" не поддерживается.';ru='Отладка обработчика ""После загрузки параметра"" не поддерживается.'");
	...
	Если Конвертация.Свойство("ПослеЗагрузкиПравилОбмена", ТекстСобытияПослеЗагрузкиПравилОбмена)
		...
		Попытка
			...
			Если ФлагРежимОтладкиОбработчиков Тогда
				...
				ВызватьИсключение НСтр("en='Отладка обработчика ""После загрузки правил обмена"" не поддерживается.';ru='Отладка обработчика ""После загрузки правил обмена"" не поддерживается.'");
		...
		Исключение
			...
			Текст = НСтр("en='Обработчик: ""ПослеЗагрузкиПравилОбмена"": %Ошибка%';ru='Обработчик: ""ПослеЗагрузкиПравилОбмена"": %Ошибка%'");
...
Процедура ПроизвестиЧтениеДанных(РезультирующаяСтрокаСИнформациейОбОшибке = "") Экспорт
	...
	Попытка
		...
		Пока ФайлОбмена.Прочитать() Цикл
				...
				Если Не ПустаяСтрока(ТекстАлгоритма) Тогда
					...
					Попытка
						...
						Если ФлагРежимОтладкиОбработчиков Тогда
							...
							ВызватьИсключение НСтр("en='Отладка обработчика ""После загрузки параметров"" не поддерживается.';ru='Отладка обработчика ""После загрузки параметров"" не поддерживается.'");
				...
				Если Не ПустаяСтрока(ТекстАлгоритма) Тогда
					...
					Попытка
						...
						Если ФлагРежимОтладкиОбработчиков Тогда
							...
							ВызватьИсключение НСтр("en='Отладка глобального алгоритма не поддерживается.';ru='Отладка глобального алгоритма не поддерживается.'");
			...
			ИначеЕсли ИмяУзла = "ДанныеПоОбмену" and Exchange Тогда
				...
				Если Не ЗначениеЗаполнено ( УзелОбменаЗагрузкаДанных ) Тогда
					ВызватьИсключение НСтр ("en='Не найден узел обмена для загрузки данных. План обмена: ';ru='Не найден узел обмена для загрузки данных. План обмена: '") + ИмяПланаОбмена + ", Код: " + КодОтКого;		
				КонецЕсли;
				...
				Если УзелОбменаЗагрузкаДанных.НомерПринятого >= мНомерВходящегоСообщения Тогда
					ВызватьИсключение НСтр("en='Номер сообщения меньше либо равен ранее принятому';ru='Номер сообщения меньше либо равен ранее принятому'");		
				КонецЕсли;
...
Процедура ВыполнитьДействияПослеЗавершенияЧтенияДанных() Экспорт
	...
	if ( Exchange and not ФлагОшибки ) then
		if ( ValueIsFilled ( УзелОбменаЗагрузкаДанных ) ) then
			objectNode = УзелОбменаЗагрузкаДанных.GetObject ();
			if ( objectNode <> Undefined ) then
				if ( objectNode.ReceivedNo <= мНомерВходящегоСообщения ) then
					objectNode.ReceivedNo = мНомерВходящегоСообщения;
					objectNode.Write ();
				endif;	
			endif; 
	...
	Если ЭтоИнтерактивныйРежим Тогда
		ТекстСообщения = НСтр("en='Загрузка данных завершена.';ru='Загрузка данных завершена.'");
...
Процедура ВыполнитьВыгрузку() Экспорт
	...
	Если ЭтоИнтерактивныйРежим Тогда
		ТекстСообщения = НСтр("en='Выгрузка данных завершена.';ru='Выгрузка данных завершена.'");
		СообщитьПользователю(ТекстСообщения);
	КонецЕсли;
...
Функция ОпределитьДостаточностьПараметровДляПодключенияКИнформационнойБазе(СтруктураПодключения, СтрокаПодключения = "", СтрокаСообщенияОбОшибке = "")
	...
	Если СтруктураПодключения.ФайловыйРежим  Тогда
		...
		Если ПустаяСтрока(СтруктураПодключения.КаталогИБ) Тогда
			...
			СтрокаСообщенияОбОшибке = НСтр("en='Не задан каталог информационной базы-приемника';ru='Не задан каталог информационной базы-приемника'");
	...
	Иначе
		...
		Если ПустаяСтрока(СтруктураПодключения.ИмяСервера) Тогда
			...
			СтрокаСообщенияОбОшибке = НСтр("en='Не задано имя сервера 1С:Предприятия информационной базы-приемника';ru='Не задано имя сервера 1С:Предприятия информационной базы-приемника'");
		...
		Если ПустаяСтрока(СтруктураПодключения.ИмяИБНаСервере) Тогда
			...
			СтрокаСообщенияОбОшибке = НСтр("en='Не задано имя информационной базы-приемника на сервере 1С:Предприятия';ru='Не задано имя информационной базы-приемника на сервере 1С:Предприятия'");
...
Функция ПодключитсяКИнформационнойБазе(СтруктураПодключения, СтрокаСообщенияОбОшибке = "")
	...
	if ( СтруктураПодключения.Property ( "Tenant" )
		and СтруктураПодключения.Tenant <> "" ) then
		СтрокаПодключения = СтрокаПодключения + ";Zn=""" + СтруктураПодключения.Tenant + """";
	endif; 
	...
	Исключение
		...
		СтрокаСообщенияОбОшибке = НСтр("en='При попытке соединения с COM-сервером произошла следующая ошибка:';ru='При попытке соединения с COM-сервером произошла следующая ошибка:'") + Символы.ПС 
						+ ОписаниеОшибки();
		
		СообщитьПользователю(СтрокаСообщенияОбОшибке);
		WriteLogEvent ( "COM.Connect.Error", EventLogLevel.Error, , , СтрокаСообщенияОбОшибке );
...
Функция ПолучитьИмяПротоколаДляВторойИнформационнойБазыComСоединения()
	...
	ИначеЕсли Не ПустаяСтрока(ИмяФайлаПротоколаОбмена) Тогда
		...
		ИмяФайлаПротоколаВыгрузки = ИмяФайлаПротоколаВыгрузки + "_Loading";
...
// Возвращает проинициализированную обработку ExchangeData базы-приемника,
...
// 
//  Возвращаемое значение:
//  ОбработкаОбъект – ExchangeData – обработка базы-приемника для загрузки данных в базу-приемник
//
Функция ВыполнитьПодключениеКИБПриемнику() Экспорт
	...
	СтруктураПодключения.Insert ( "Tenant", Tenant );
	...
	Попытка
		РезультатПодключения = ОбъектПодключения.Обработки.Exchange.Создать();
	Исключение
		
		Текст = НСтр("en='При попытке создания обработки ExchangeData произошла ошибка:';ru='При попытке создания обработки ExchangeData произошла ошибка:'") + КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
		СообщитьПользователю(Текст);
		РезультатПодключения = Неопределено;
	КонецПопытки;
	...
	Если РезультатПодключения <> Неопределено Тогда
		...
		РезультатПодключения.SenderCode = SenderCode;
		РезультатПодключения.SenderName = SenderName;
...
Function GetRecordKey ( Records ) export
	
	fields = new Structure ();
	for each item in Records.Filter do
		fields.Insert ( item.Name, item.Value );
	enddo; 
	name = Records.Metadata ().Name;
	rk = InformationRegisters [ name ].CreateRecordKey ( fields );
	return rk;
	
EndFunction 

Procedure DeleteRecord ( НаборЗаписей, Объект ) export
	
	meta = НаборЗаписей.Metadata ();
	name = meta.Name;
	r = InformationRegisters [ name ].CreateRecordSet ();
	УстановитьОбменДаннымиЗагрузка ( r );
	for each dim in meta.Dimensions do
		r.Filter [ dim.Name ].Set ( Объект [ dim.Name ] );
	enddo; 
	r.Write ();
	
EndProcedure 

Procedure RefreshPredefined () export
	
	for each row in ТаблицаПравилКонвертации do
		elements = row.Значения.Count ();
		if ( elements = 0 ) then
			continue;
		endif; 
		values = new Map ();
		error = false;
		for each item in row.Значения do
			try
				ref = Менеджеры [ row.Источник ].Менеджер [ item.Value ];
				values [ ref ] = item.Value;
			except
				// Dot not remove values from the row.Значения!
				// The next tenant should have list of predefined names
				error = true;
				break;
			endtry;
		enddo; 
		if ( not error
			and elements = values.Count () ) then
			row.Значения = values;
		endif; 
	enddo; 
...
// Exchange
Процедура ЗаписатьИнформациюОбОбменеДаннымиЧерезПланыОбмена ( УзелДляОбмена )
