...
Процедура ИмяФайлаВыгрузкиНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ДиалогВыбораФайла.Show ( new NotifyDescription ( "FileSelected", ThisObject ) );

КонецПроцедуры

&AtClient
Procedure FileSelected ( File, Params ) export
	
	if ( File = undefined ) then
		return;
	endif;
	ИмяФайлаВыгрузки = File [ 0 ];
...
Процедура Выгрузить(Команда)
	...
	Состояние(Нстр("ru='Выполняется выгрузка описания структуры метаданных конфигурации. Пожалуйста, подождите...';='Выполняется выгрузка описания структуры метаданных конфигурации. Пожалуйста, подождите...'"));
...
Процедура ИзменитьРежимОбработки()
	...
	?(ЭтоКлиент, НСтр("ru='Режим работы (на клиенте)';='Режим работы (на клиенте)'"), НСтр("ru='Режим работы (на сервере)';='Режим работы (на сервере)'"));
