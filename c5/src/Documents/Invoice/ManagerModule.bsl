...
Procedure sqlPrintData ( Env )
	...
	|	Document.Company.Discounts as Discounts, Document.Tax as Tax, Document.Amount - Document.Tax as Subtotal,
	|	Document.Creator.Description as Salesman, Document.Customer.FullDescription as Customer, Document.PO as PO, Document.PaymentDate as PaymentDate,
	|	Document.Contract.CustomerPayment as CustomerPayment, Document.Contract.CustomerTerms.Description as CustomerTerms,
	...
	|	case when Document.Shipment = value ( Document.Shipment.EmptyRef ) then """" else Document.Shipment.Number end as ShipmentNumber,
	|	Document.Customer.PaymentAddress.Presentation as PaymentAddress, 
	|	Document.Customer.ShippingAddress.Presentation as ShippingAddress,
	|	Document.Contract.CustomerTerms.Description as Terms, Document.Taxable as Taxable,
	|	Document.Amount as Amount, Document.Discount as Discount, Document.GrossAmount as GrossAmount, 
	|	Document.Currency.Description as Currency, Document.Memo as Memo
	|from Document.Invoice as Document
	...
	|	Items.Price as Price, Items.Amount as Amount, Items.Discount as Discount, ( Items.QuantityPkg * Items.Price ) as GrossAmount,
	...
	|;
	|// #Services
	|select Services.Item.Description as Item, Services.Item.Code as Code, Services.Feature.Description as Feature, Services.Quantity as Quantity,
	|	"""" as Package, Services.Price as Price, Services.Amount as Amount, Services.Discount as Discount, ( Services.Quantity * Services.Price ) as GrossAmount,
	...
	|from Document.Invoice.Taxes as Taxes
	|where Taxes.Ref = &Ref
...
Procedure putHeader ( Params, Env )
	...
	p.SalesOrderNumber = Print.ShortNumber ( fields.SalesOrderNumber );
	p.ShipmentNumber = Print.ShortNumber ( fields.ShipmentNumber );
...
Procedure putFooter ( Params, Env )
	...
	if ( fields.Discounts ) then
		...
		p = area.Parameters;
		p.GrossAmount = fields.GrossAmount;
		p.Discount = fields.Discount;
		tabDoc.Put ( area );
	endif; 
