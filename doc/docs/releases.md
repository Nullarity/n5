...
1. Улучшена точность расчета сумм при продажах в условных единицах.
2. Реализована возможность выбора виртуальных диапазонов без фильтрации по складу.
3. Исправлена ошибка определения конца электронных диапазонов.
4. Кнопка `Создать` удалена из журнала `Автосписания бланков`, так как этот вид документов не создается интерактивно.
5. Реализована возможность ежемесячной рассылки расчётных листков.
6. Фильтр по статусу налоговой накладной в журнале налоговых накладных исправлен и теперь работает корректно.
7. В справку документов блока диапазонов добавлена ссылка на документацию по диапазонам.
8. Флаг `Показывать в списке` теперь выключается автоматически при создании пользователя в облачном режиме.
9. При регистрации диапазона, номер документа теперь скрыт, чтобы не вводить пользователя в заблуждение.
10. В отчет `Расчетная ведомость` добавлена возможность отбора по территориальному подразделению.
11. Теперь проводки с нулевыми суммами в документе `Закрытие полученных авансов` не формируются.
12. Теперь система предлагает устанавливать флаг `Подписан` для подчиненных договоров.
13. Теперь сортировка информации в отчете `Акт сверки` производится в порядке возрастания дат действия договоров.

# 5.0.33.1 от 15.05.2023
...
21. Остаток товара в возврате поставщику теперь рассчитывается корректно, даже если товар введен в поступлении более одного раза.
