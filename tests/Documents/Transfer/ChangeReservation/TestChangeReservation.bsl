...
MainWindow.ExecuteCommand ( "e1cib/list/Document.Transfer" );
With ( "Transfers" );
Click ( "#FormCreate" );
formTransfer = With ( "Transfer (create)" );
Put ( "#Sender", env.WarehouseB );
Put ( "#Receiver", env.WarehouseA );
...
Function getEnv ()
	...
	env = new Structure ();
	id = Call ( "Common.ScenarioID", "#2513C54E" );
	env.Insert ( "ID", id );
	date = CurrentDate ();
