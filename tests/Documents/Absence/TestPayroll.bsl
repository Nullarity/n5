...
id = Call ( "Common.ScenarioID", "27E7C07C" );
env = getEnv ( id );
createEnv ( env );
...
Check ( "#Compensations / Result [ 1 ]", 363.64 ); // For rate = 1000
Check ( "#Compensations / Days [ 1 ]", 8 ); // For rate = 1000
Check ( "#Compensations / Hours [ 1 ]", 64 ); // For rate = 1000

Check ( "#Compensations / Result [ 2 ]", 68.18 ); // For rate = 1500
Check ( "#Compensations / Days [ 2 ]", 1 ); // For rate = 1500
Check ( "#Compensations / Hours [ 2 ]", 8 ); // For rate = 1500

Check ( "#Compensations / Result [ 3 ]", 681.82 ); // For rate = 1500
Check ( "#Compensations / Days [ 3 ]", 10 ); // For rate = 1500
Check ( "#Compensations / Hours [ 3 ]", 80 ); // For rate = 1500

Check ( "#Compensations / Result [ 4 ]", 1000 ); // Rate has not been changed
...
Procedure createEnv ( Env )
	...
	formAbsence = With ( "Absence at Work (cr*" );
	table = Get ( "#Employees" );
	employee = env.Employees [ 0 ].Name;
	Click ( "#EmployeesContextMenuAdd" );
	Put ( "#EmployeesEmployee", employee, table );
	Put ( "#EmployeesDateStart", "01/12/2017", table );
	Put ( "#EmployeesDateEnd", "01/13/2017", table );

	Click ( "#EmployeesContextMenuAdd" );
	Put ( "#EmployeesEmployee", employee, table );
	Put ( "#EmployeesDateStart", "01/17/2017", table );
	Put ( "#EmployeesDateEnd", "01/17/2017", table );
	Click ( "#FormPostAndClose" );
