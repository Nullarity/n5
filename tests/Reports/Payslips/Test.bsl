...
id = Call ( "Common.ScenarioID", "2BE5DC31" );
env = getEnv ( id );
createEnv ( env );

p = Call ( "Common.Report.Params" );
p.Path = "Employees / Payslips";
p.Title = "Payslips*";
filters = new Array ();
item = Call ( "Common.Report.Filter" );
item.Period = true;
item.Name = "Period";
item.ValueFrom = "01/01/2019";
item.ValueTo = "03/31/2019";
filters.Add ( item );
item = Call ( "Common.Report.Filter" );
...
Procedure createEnv ( Env )
	...
	p = Call ( "CalculationTypes.Compensations.Create.Params" );
	mainCompensation2 = "_Additional: " + id;
	p.Description = mainCompensation2;
	p.Method = "Monthly Rate";
	Call ( "CalculationTypes.Compensations.Create", p );
	...
	Click ( "#CompensationsContextMenuCopy" );
	With ( "Compensation" );
	Put ( "#Compensation", mainCompensation2 );
	Put ( "#AccountingResult", "1000" );
	Click ( "#FormOK" );
